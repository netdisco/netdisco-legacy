...
Miller, Bill Fenner, Oliver Gorwits, Jeroen van Ingen and Max Baker.
...
SECONDS. Prevent a device from being arpnipped for this number of seconds after the 
last succesful run.
...
=item discover_min_age

SECONDS. Prevent a device from being refreshed for this number of seconds after the 
last succesful run.
...
=item macsuck_min_age

SECONDS. Prevent a device from being macsucked for this number of seconds after the 
last succesful run.
...
=item macsuck_no_devicevlan

LIST:Strings.  Comma separated list of "hostname:VLAN" combinations to ignore when MACsucking.

This option is similar to L<macsuck_no_vlan>, but only skips MACsucking for the given
VLAN on the given device.
...
=item macsuck_no_unnamed

BOOLEAN.  Set to true to skip MACsuck-ing on VLANs which have no name set.

This option may be useful on Cisco Catalyst family devices where ports are a
member of a VLAN which is not defined in the VLAN database.
...
=head3 Device View preferences

=over

=item col_xxx_show

BOOLEAN. Customize which columns are shown by default in de Device View page.
The "xxx" part indicates the (internal) column name. Valid options are:
"port", "descr", "type", "duplex", "lastchange", "name", "speed", "mac", "mtu",
"vlan", "vmember", "connected", "stp", "up", "control", "graphs", "poe_admin",
"poe_status", "poe_class" and "poe_power".
...
=item col_graphs_data

FILE. Should resolve to a Mason component which provides output for the Graphs column.
An example component named "col_graphs" is included in html/graphs_sample.mas.

Example:
    col_graphs_data = graphs_sample.mas:col_graphs

Linking Netdisco against external graphing tools requires a small amount of 
programming. This is not in the scope of this manual.
...
=item device_port_power

Holds PoE-related information for interfaces (ports) on each device capable of acting 
as Power Sourcing Equipment (PSE).  One row for each power-capable interface exists with 
information about the PoE status.
