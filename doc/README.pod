...
Database cleanup routines (L<-K>) as well. 
...
For a full backup run F<sql/pg_back> to backup the whole database.
...
To refresh or discover a device and its ports, use the L<-d> command:
...
however restart the admin daemon.  The topology text file is reparsed each time you run netdisco.

As a convience you can use the topology file to quickly seed Netdisco with devices. 
...
The expire data routines are called from the L<-B>/Backup routine, which should
...
which ports are uplink ports and which have connected nodes.  Ports that are uplink ports
that are not marked so in Netdisco will appear to B<steal> MAC address entries from their 
rightful ports.  So it is critical to use the topology file and CDP to maintain a topology.


=head3 Manual Topology Information

Netdisco will auto-discover the layer-two topology of a network using CDP.  However, many networks 
...
Mark Boolootian (Through who's ideas Netdisco was born and shaped)  (UCSC), Jim Warner (UCSC), 
Mike Hunter (UCB), Brian Wilson (NCSU), David Temkin (sig.com), Edson Manners (FSU), Dmitry Sergienko (Trifle Co, .ua)
Remo Rickli (PSI, Switzerland), Jean-Philippe Luiggi (sagem.com), 
A.L.M Buxey (Loughborough University, UK), Kevin Cheek (UMICH), Bradley Baetz (bbaetz),
John Bigrow (bnl.gov), George Pavel (llnl.gov), Charles Goldsmith (CompUSA)
