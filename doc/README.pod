...
=item * Search by OUI (MAC Address Vendor)

=item * Search for Wireless Access Points (APs) from wired-side of network

=item * Search for Device Ports with multiple Nodes attached

=item * Search for Nodes with multiple IP addresses

=item * Search for Devices using IP's w/out DNS entries
...
=item - MIBS              (SNMP MIBs for your devices)
