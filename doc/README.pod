...
Calls database cleanup routines (-K) as well.
...
PATH. Full path to the directory that backups and logs will be stored in
...
FILE. Full path and name to the ISMAP data for the network.  Path should be the 
...
STRING. The X and Y dimensions of the graph in inches.   To convert to pixels, times by 
100 (96 actually).  So 30x30 will give you a graph that is about 3000x3000 pixels wide.

=item graph_fontpath

PATH. Full path to the True Type Font (TTF) used in L<node_font>.
...
FILE. Name of the True Type Font used for label of node.  See L<graph_fontpath> above.
Exclude .ttf in name.
