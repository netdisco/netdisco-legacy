...
The C<pg_init> and C<pg_run> commands use F<netdisco.conf> to find the database
...
F<netdisco_apache.conf> and in F<netdisco.conf>.
...
C<pgsql>.  If yours is different, please change it appropriately in
F<sql/pg_init>.

=item [4.6] Check Settings

Now as a non-privileged user (root) make sure you can get into the database by
running C<sql/pg_run>.  The database user password given above will be
...
=item [4.10] Postgres Maintenance
...
=item [4.11] Remote Database (Optional)
...
Add the following two lines in your httpd.conf before the includes from
step 7.3.3:
