...
Developed by team of Open Source authors headed by Eric Miller, Bill Fenner,
and Oliver Gorwits, originally created by Max Baker.  See list at the end of
README of all the contributors.
...
The Netdisco project is hosted by Source Forge at L<https://sourceforge.net/projects/netdisco>
and L<https://sourceforge.net/projects/snmp-info>.
...
L<https://sourceforge.net/tracker2/?group_id=80033&atid=558508>
...
L<https://sourceforge.net/tracker2/?group_id=80033&atid=558510> and join us on
the C<netdisco-devel> mailing list.

=item To register your install of Netdisco
...
let us know you're using it at L<http://netdisco.org/register.html>.

=back

=head1 INSTALL FLAVORS
...
The most common way is to install it from source on your machine using an
install script (see F<bin/install> and
F<http://www.auburn.edu/~gouldwp/netdisco/>), or manually using this document.
...
going to the trouble of installing it manually.

=item Virtual Machine / VMWARE Applicance

Charles Goldsmith maintains a VMWARE Appliance version of Netdisco
at L<http://wokka.org/netdisco/>.  This is by far the easiest way
to get up and running w/ Netdisco. 
...
5.8.x and newer advised.   Whatever version that comes with your distro should
be fine.
...
1.26 or newer required.  1.39 or newer advised, especially
...
If you have freshly installed Postgres then the default memory usage B<NEEDS> to
...
On the Postgres machine, allow your remote front end to contact the back-end
...
Please use the older working versions 5.2.2 or 5.3.0.1, or try the
newer 5.2.4, 5.3.2 or 5.4 when they're available.  See
L<http://www.net-snmp.org/dev/schedule.html> for release schedule
plans.  You can also use a nightly version from the appropriate
patch branch, from L<http://www.net-snmp.org/nightly/tarballs/>.

The symptom of the failure is getting an error C<Not an ARRAY reference at SNMP/Info.pm line xxx>.  

A workaround if you must use a buggy Net-SNMP version is to use SNMPv1 or turn
off bulkwalk with C<bulkwalk_off: yes> -- but the best thing to do is change your
Net-SNMP version.
...
Net-SNMP version - i.e., if you get a warning like this when you try to install:
...
change for these tools.   Usually the settings in F<netdisco.conf> are more
than enough.

    snmpconf

Make sure to set the MIB dir if non-standard or you are not using the MIBS that
come with Netdisco.
...
F<mibs/> directory if you installed the C<netdisco-1.0_with_mibs> package.
...
Make sure that C<neato> or C<twopi> is in your path, and in the path
in F<netdisco.crontab>.
...
Comment out the lines at the top of F<netdisco_apache.conf> that
...
Some distros use the name C<apache> or C<www> as the web server unix user.
...
Use this crontab to control the frequency of data collection done by Netdisco.
...
If you run into errors because of UTF-8 or other exotic characters
in oui.txt you may need to add this line to F<netdisco.conf>.

    db_Pg_env = PGCLIENTENCODING => iso8859-1

You may also try changing the above from C<iso8859-1> to C<UTF-8>.
...
Now, as C<root>, load the crontab for the C<netdisco> user.
...
web front-end.  See F<README> for a full description of the admin daemon.
...
Here are some notes for installing Netdisco on Linux.  If your distro is not
listed here, look over the install scripts for other distros at
L<http://www.auburn.edu/~gouldwp/netdisco/> in the READMEs section.
...
=head2 Fedora / CentOS / Redhat

Netdisco 0.95 packages are available in the Fedora 11 repositories, and the
EPEL repositories for RHEL/CentOS 5.

Newer releases may be available from the package author's website, at
L<http://www.auburn.edu/~gouldwp/netdisco>.

We expect RPMs for Netdisco 1.0 and its dependencies to appear in the Fedora
and EPEL repositories in due course.

=head2 Mandrivia Linux

An RPM is available for Mandrivia Linux, but may be out of date.  Please check
that the RPM for Netdisco 0.95 is marked at least 200611 or newer.

=head2 Debian Linux

Debian 5.0 (Lenny) contains the Netdisco 0.95 packages and all dependencies.
Newer releases may be available at L<http://packages.debian.org/netdisco>.

We expect packages for Netdisco 1.0 and its dependencies to appear in the
official repositories for Debian "Squeeze" in due course.
...
=head2 Ubuntu 8.04 (Hardy)

There is an old version of Netdisco in aptitutde.  So what we'll do is use that to install
all the dependencies, then uninstall netdisco and use a version from Source.

=over

=item aptitude install netdisco-backend netdisco-frontend
...
=item Now remove netdisco-backend netdisco-frontend netdisco-common and libnetsnmp

BUT keep all the dependencies.

=item Install Source

Grab netdisco-with-mibs from L<https://sourceforge.net/projects/netdisco/files/>  and install into /usr/local/netdisco.

=item Install SNMP::Info

=item Change database passwords in F<netdisco.conf> and F<netdisco_apache.conf>
...
=item Edit F</etc/postgresql/8.3/main/pg_hba.conf> and relax all security for now.
...
=item Run C</etc/init.d/postgresql-8.3 restart>

=item Run /usr/local/netdisco/sql/pg --init

=item Go back into F<pg_hba.conf> and reinstate security
...
=item Create an SSL certificate

 mkdir /etc/apache2/ssl 
 /usr/sbin/make-ssl-cert /usr/share/ssl-cert/ssleay.cnf /etc/apache2/ssl/apache.pem 

=item Turn on a couple Apache modules you installed

 a2enmod ssl
 a2enmod apreq

=item Create an apache config file to load Netdisco
...
Make sure to turn on the secure server option in C<netdisco.conf>.

=item Run C</etc/init.d/apache2 restart>
...
Once you have or have not succeeded, come join the Netdisco User's mailing list
at L<http://netdisco.org> and pass on any hints you may have in the INSTALL
