...
This is not a terribly easy install.  If you aren't comforatable 
...
=head2 5. Install Perl Modules and Mason
...
=item ii. Install SNMP to include Perl module that comes inside the distribution.
...
For BSD use ports: C<cd /usr/ports/graphics/graphviz && make install>

Else get the newest version from L<http://www.graphviz.org/pub/graphviz>
...
=head2 9. Modify F<netdisco.conf> to match your site.  

See F<README> for detailed configuration descriptions.

=head2 10. Add a Web Admin User Inside Netdisco

    /usr/local/netdisco/netdisco -u username

=head2 11. Setup Apache
...
enviornment.  Due to the security and privacy concerned associated with
...
=item b. cd /usr/ports/security/openssl & make insatll
...
Add this line in the end of C<httpd.conf> not in a VirtualHost section.

    Include /usr/local/netdisco/netdisco_apache.conf

=item b. In each VirtualHost
...
=item ii. Create Docs
 ...
 cd /usr/local/netdisco && make docs
