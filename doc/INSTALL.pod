...
Developed by team of Open Source authors headed by Eric Miller and
Bill Fenner.
...
Please use the C<netdisco-users> mailing list for all help, problems and comments.
...
Please use 5.2.2 or 5.3.0.1, or (when they come out)
5.2.4 or 5.3.2.  The symptom of the failure is getting
an error "Not an ARRAY reference at SNMP/Info.pm line xxx".
A workaround if you must use a buggy Net-SNMP version is
to use SNMPv1 or turn off bulkwalk with bulkwalk_off: yes.
...
B<Do not install version 1.99>.  For apache2 support
make sure you have mod_perl 2.02 or later.
...
See L<INSTALL|install> below for details on how to retrieve and install each one.
...
have been reported under Redhat, Mandrivia, Gentoo and Debian.
A beta RPM is available for Mandrivia.

One user has reported getting Netdisco running on OS X.  Solaris has been
successful for 8 and 10.  See
L<http://www.saintmarys.edu/~dmckeown/netdisco_solaris10.htm>.
...
Modify the following shell scripts to point to your non-standard
netdisco directory:
...
Edit the files C<pg_init> and C<pg_run> to match your database user name in F</usr/local/netdisco/sql/pg_init>
...
Jordi Guijarro reported that the version that comes with Woody is too old.  Install a newer
version by hand.
...
Apache2.  One reported success uses mod_perl 2.0.2 and Mason 1.32.
See configuration details under [7.4.4].
...
=head3 7.3.  Apache 1.3.x From Scratch
...
above.  If you suspect yours is not compiled in, run C<httpd -l> and see if 
C<mod_perl.c> is listed (it should be).
...
=head3 8.2.  Non-Standard Perl Location

If you don't have Perl installed in C</usr/bin>, you will need to modify
the first lines of F<netdisco> and F<bin/doc_munge>.
...
This is where you control the frequency of data collection done by Netdisco.
...
You can do this step if you would like the SNMP documentation available from the
web as well.
...
=item [11.2] Use the Apache installed by Mandrake


    /etc/rc.d/init.d/httpd restart

=item [11.3] Use the Apache installed by BSD Ports
...
If your network is CDP/FDP/SONMP aware, pick a device that you consider close
to the center of the network and start an auto-discovery from there:
...
configuration file.  A full restart is required.
...
The daemon will be respawned daily in the Cron job listed above.  This isn't 100%
necessary, I just did in case the net-snmp libraries had any memory leaks.

=head3 16.1 Make the Admin Daemon start at every boot
