...
and includes specific notes to install with it.
...
F<pg_hba.conf> file to take precedence.  Note that if you are using Postgres 7.2 you
may have to change C<md5> to C<crypt>, Actual mileage may vary.
...
If there are problems with this step you will receieve an error that mentions something
about F<pg_hba.conf>.

=item ii. Create Database.
...
=item Install Apache1

Mount CD1 and run as root:

    rpm -Uvh /mnt/cdrom/Mandrake/RPMS/libmm1-1.2.2-2mdk.i586.rpm 
    rpm -Uvh apache-devel-1.3.27-8mdk.i586.rpm libmm1-devel-1.2.2-2mdk.i586.rpm libgdbm2-devel-1.8.0-19mdk.i586.rpm 
    rpm -Uvh mod_perl-devel-1.3.27_1.27-7mdk.i586.rpm 
    rpm -Uvh db1-devel-*

Mount CD2 and run as root :

    cd /mnt/cdrom/Mandrake/RPMS2
    rpm -Uvh mod_perl-common-1.3.27_1.27-7mdk.i586.rpm apache-mod_perl-1.3.27_1.27-7mdk.i586.rpm apache-modules-1.3.27-8mdk.i586.rpm 

Need this for DB_File perl module

    ln -s /usr/include/db1/* /usr/include
    ln -s /usr/lib/libdb1.so.2 /usr/lib/libdb.so

=item Configure Both Apaches

Configuration files for Mandrake live in F</etc/httpd/conf>

In the file F<commonhttpd.conf> add the two C<Include> lines:
...
In the file F<httpd2.conf> add the final C<RewriteRule> line:
...
used in this version of OpenSSL:
    ...
    export blahblahblah (TODO)
    ./configure \
        --with-apache=../apache_1.3.x \        
        --disable_rule=SSL_COMPAT

=item By Hand (other OSs)
...
=head2 9. Perl Modules and Mason
...
It's a good place to keep up on new features and releases, and it's the best
