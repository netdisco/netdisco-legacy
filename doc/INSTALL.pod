...
=item * PostgreSQL (7.3   and newer advised)

=item * Mason      (1.16 or newer required)
...
Username: C<netdisco>
Database Name: C<netdisco>

=over 

=item 1. [Optional] Change database name in F<sql/pg_init>

=item 2. [Optional] Change database name in F<sql/pg_run>
...
=item 6. [Optional] Permissions

If you have just setup postgresql, you may want to change the
default permissions in F<data/pg_hba.conf> :

=item 7. [Optional] Optimizations

If you have freshly installed Postgresql then the default memory usage should
...
=item In Each VirtualHost (SSL,Main...)

C<Include /usr/local/netdisco/netdisco_apache_dir.conf>

=item In the Global Section
...
Assuming C<nobody> is the user that your HTTP process run under,
run this as C<root> :

C<chown nobody.netdisco /usr/local/netdisco/mason>

=item 4. [Optional] In F<netdisco_apache.conf> and F<netdisco_apache_dir.conf>

=over

=item *  Change the web and physical paths appropriately.
...
=item 4. Make sure that neato/twopi is in your path, and/or in the crontab path
