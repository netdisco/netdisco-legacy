...
Netdisco 0.93 (Beta/CVS)

=head1 AUTHOR

Originally created by Max Baker,  See list at the end of README of all
the contributors.

=head1 CURRENT VERSION

It is a good idea to check the latest version of this document
at L<http://netdisco.org> for corrections and additions.
...
have been reported under Redhat, Mandrake and Debian.
...
default permissions in F<$PG_DATA/pg_hba.conf>.  $PG_DATA might be in L</usr/local/pgsql> or in
L</var/db>, depends on your OS.  Check the postgres users' homedir files for the location of $PG_DATA.
...
You may have to change C<md5> to C<crypt>, Actual mileage may vary.
...
=item iii. [OPTIONAL] Configure Net-SNMP for a non-standard setup
...
with C<force install ModuleName>
...
Once you have or have not succeded, come join the Netdisco User's mailing list at 
